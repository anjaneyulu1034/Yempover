...
class MyProfileService {
  ...
  Future<GetMyProfileResponse> getMyProfile({int page = 1, int limit = 20}) async {
    try {
      ...
      debugPrint('📨 MyProfileService: Response status: ${response.statusCode}');

      if (response.statusCode == 200 || response.statusCode == 201) {
        final Map<String, dynamic> jsonResponse = json.decode(response.body);
        debugPrint('✅ MyProfileService: profile fetched successfully ${jsonEncode(jsonResponse)}');
